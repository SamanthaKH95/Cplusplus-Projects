...
void Date::setDate(int yyyy, int mm, int dd)
{
    try
    {
        if (yyyy > YEAR_MAX || yyyy < YEAR_MIN)
        {
            ...
            throw invalid_argument ("The year value inputs are not within valid range");
        }
        
        if (mm > MONTH_MAX || mm < MONTH_MIN)
        {
            this->mm = MONTH_DEFAULT;
            throw invalid_argument ("The month value inputs are not within valid range");
        }
        
        if ((mm == 2 && dd > 29) || (mm == 2 && dd < 1))
        {
            this->dd = DAY_DEFAULT;
            throw invalid_argument ("The day value inputs are not within valid range for month");
        }
        
        if (((mm == 4 || mm == 6 || mm == 9 || mm == 11) && dd > 30) ||
            ((mm == 4 || mm == 6 || mm == 9 || mm == 11) && dd < 1))
        {
            this->dd = DAY_DEFAULT;
            throw invalid_argument ("The day value inputs are not within valid range for month");
        }
    ...
    {
        cout << "Invalid Argument Exception caught in Date::setDate" << endl;
        cout << e.what(); 
    }
